...
def update_intervention_location(intervention_Id):
    can_not_edit=get_incidents_by_status('intervention',int(intervention_Id))
    incident=update_incident_location('intervention',int(intervention_Id))
    if can_not_edit:
        return can_not_edit
    elif incident:
...
def update_intervention_comment(intervention_Id):
    can_not_edit=get_incidents_by_status('intervention',int(intervention_Id))
    incident=update_incident_comment('intervention',int(intervention_Id))
    if can_not_edit:
        return can_not_edit
    elif incident:
        return jsonify({"status":200,"Data":[incident,
