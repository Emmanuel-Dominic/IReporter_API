...
from api.controllers.incident_controller import mailme,get_incidents_by_status,get_incidents_by_type,get_incidents_by_type_id,create_incident,update_incident_comment,update_incident_location,update_incident_status,delete_incident
...
def get_specific_redflag(redflag_Id):
    intervention=get_incidents_by_type_id('redflag',int(redflag_Id))
    if intervention:
        return jsonify({"status": 200, "data": intervention}), 200
    return not_found()
...
def update_redflag_location(redflag_Id):
    not_incident_status=get_incidents_by_status('redflag',int(redflag_Id))
    incident=update_incident_location('redflag',int(redflag_Id))
...
def update_redflag_comment(redflag_Id):
    not_incident_status=get_incidents_by_status('redflag',int(redflag_Id))
    incident=update_incident_comment('redflag',int(redflag_Id))
    if not_incident_status:
        return not_incident_status
    if incident:
        return jsonify({"status":200,"Data":[incident,
...
def delete_redflag(redflag_Id):
    not_found_id=get_incidents_by_type_id('redflag',int(redflag_Id))
